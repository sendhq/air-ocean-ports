...
const searchAirPortsByName = (name) => {
  return airports.filter((airline) => airline.name.toLowerCase().includes(name.toLowerCase()));
};
...
const searchAirPortsByIATACode = (code) => {
  return airports.filter((airline) => airline.iata_code.toLowerCase().includes(code.toLowerCase()));
};
